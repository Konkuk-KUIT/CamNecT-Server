...
class CommunityPostAccessPolicyTest {
    ...
    void acceptedQuestionOwnerAnswerAuthorAndPurchaserAreGranted() {
        ...
        CommunityPostAccessPolicy.AccessDecision owner = policy.evaluate(1L, post, true);
        when(acceptedCommentsRepository.existsByPost_IdAndComment_UserId(10L, 3L)).thenReturn(true);
        CommunityPostAccessPolicy.AccessDecision answerAuthor = policy.evaluate(3L, post, true);
        when(postAccessRepository.existsByPost_IdAndUser_UserId(10L, 2L)).thenReturn(true);
        CommunityPostAccessPolicy.AccessDecision purchaser = policy.evaluate(2L, post, true);
    ...
    void acceptedQuestionNonPurchaserGetsBalanceBasedStatus() {
        ...
        CommunityPostAccessPolicy.AccessDecision enough = policy.evaluate(2L, post, true);
        CommunityPostAccessPolicy.AccessDecision insufficient = policy.evaluate(3L, post, true);
    ...
    void unreadableAcceptedQuestionIsForbiddenButUnacceptedQuestionIsReadable() {
        ...
        assertDoesNotThrow(() -> policy.requireReadable(2L, post));
        CustomException exception = assertThrows(CustomException.class,
                () -> policy.requireReadable(2L, post));
