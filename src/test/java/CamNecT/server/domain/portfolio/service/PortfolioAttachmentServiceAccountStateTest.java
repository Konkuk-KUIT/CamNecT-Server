...
class PortfolioAttachmentServiceAccountStateTest {
    ...
    private PortfolioAttachmentService service;

    @BeforeEach
    void setUp() {
        service = new PortfolioAttachmentService(
                accountAccessGuard,
                presignEngine,
    ...
    private void invoke(UploadKind uploadKind) {
        switch (uploadKind) {
            ...
            case ASSET -> service.presignAssetsBatch(
                    1L,
                    1L,
                    new PresignUploadBatchRequest(List.of(
                            new PresignUploadBatchRequest.Item("application/pdf", 100L, "portfolio.pdf")
                    ))
            );
        }
    }
